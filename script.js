...
description.innerText = `To See Content Using Navigation`;

var pagination = document.createElement("div");
pagination.className = " pagination table-responsive";
document.body.append(title, description, pagination);

pagination.innerHTML = `<table class="table table-bordered"   cellpadding="1">
<thead>
<tr>
<th>Id</th>
<th>Name</th>
...
buttons.className = " btns d-flex justify-content-center buttons";
buttons.id = "buttons";
data.forEach((e, i) => {
  if (i < 10) {
    document.querySelector(".table_tbody").innerHTML += `
    <tr>
    <td>${e.id}</td>
    <td>${e.name}</td>
    <td>${e.email}</td>
...
buttons.innerHTML += `<input type="button"  class="first_btn" onclick="first(event)" value="First"/>`;
buttons.innerHTML += `<input type="button"  class="next_btn" onclick="next(event)" value="Next"/>`;
let len_data = Math.ceil(data.length / 10);
for (var i = 1; i <= len_data; i++) {
  buttons.innerHTML += `<input type="button"  class=" btn btn${i}" onclick="pages(event)" value="${i}"/>`;
}
buttons.innerHTML += `<input type="button"  class="prev_btn" onclick="previous(event)" value="Previous"/>`;
buttons.innerHTML += `<input type="button"  class="last_btn" onclick="last(event)" value="Last"/>`;
document.body.append(buttons);

var data_sliced = "";
function pages(event, next_val) {
  let value;
  if (event == "") {
    value = next_val;
  } else {
  ...
  data_sliced = data.slice((value - 1) * 10, value * 10);
  document.querySelector(".table_tbody").innerHTML = "";
  data_sliced.forEach((e) => {
    document.querySelector(".table_tbody").innerHTML += `<tr>
      <td>${e.id}</td>
      <td>${e.name}</td>
      <td>${e.email}</td>
    </tr>
...
function last(event) {
  ...
  data_sliced = data.slice(data.length - 10, data.length);
}

function next() {
  if (data_sliced.length <= 0) {
    return;
  } else {
    let Lst_value = parseInt(data_sliced[data_sliced.length - 1].id);
    if (Lst_value < 100) {
      pages("", Lst_value / 10 + 1);
    } else {
      return;
    }
  }
...
function previous() {
  ...
  {
    let Lst_value = parseInt(data_sliced[0].id);
    if (Lst_value > 1) {
      pages("", (Lst_value - 11) / 10 + 1);
    } 
    else 
    {
      return;
